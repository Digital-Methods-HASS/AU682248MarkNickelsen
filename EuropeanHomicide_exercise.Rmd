...
Konger <- read_delim("data/Kongerækken.csv")
```
...
Konger %>% 
  select(Navn,Fra,Til) %>% 
  mutate(Total_regeringstid=Til-Fra) %>% 
  ggplot(aes(x=reorder(Navn,Fra),y=Total_regeringstid))+
  geom_col()+
  labs(title="Regeringstid pr Konge",
       y="Aar", x="Konge")+
...
ggplot(data = Western_Europe,aes(x=Year,y=homicides_per_100k))+
  geom_col(aes(group=Entity,colour = factor(Entity)))+
  labs(title = "Homicide over Time",x="Timeline",y="Homicide rate pr. 100k", colour="European regions")
  #....YOUR CODE GOES HERE
```
...
ggplot(data = Western_Europe, aes(x=Year,y=homicides_per_100k)) +
  ...
  labs(title = "Homicides tover Time",y="Homicide rate pr. 100k", colour="European regions")
```
...
```{r mutating to create a new colum, eval=TRUE}
Kunger<-Konger %>% 
mutate(Reign_duration=Til-Fra) %>% 
mutate(Midyear=Til-(Til-Fra)/2)
```
...
ggplot(aes(x=Midyear,y=Reign_duration))+ 
  geom_smooth()+
  labs(title="Reign Length",x="Middle of Reign", y="Duration")
```
As the homicide rate lowers, the duration of reign of Monarchs increases.
...
ggplot(data = Western_Europe) + 
  ...
  labs(title = "Homicides over time", y = "Homcides pr 100k people", colour = "Country")
```
...
Looking at the falling homicide rates and the increasing reigntimes of kings, we can 
say we might be more civilized today
